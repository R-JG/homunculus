...
+$  zona                                                               :: terminal input, full
  ...
  $%  [%rez p=@ud q=@ud]                                               ::
      ...
      [%esc ~]                                                         ::
  ==                                                                   ::
+$  nota                                                               :: terminal input, as keys to be mapped to events
  $~  [%txt ~]                                                         ::
  $%  [%clk ?(%d %u)]                                                  ::
      [%whe ?(%d %u)]                                                  ::
...
+$  omen                                                               :: keybindings per mode
  $:  element=(map nota element:lex)                                   ::
      command=(map nota command:lex)                                   ::
      leader=(map nota leader:lex)                                     ::
      insert=(map nota insert:lex)                                     ::
...
++  on-load
  |=  vaz=vase
  ^-  (quip card _hoc)
  =/  old  (mole |.(!<([%test deus] vaz)))
  =.  ego
    ?~  old
      ~&  >>>  %resetting-state
      %_  ego
        viae       ~[*via]
        deus.urbs  (volo [100 25])
        omen       scio
      ==
    %_  ego
      viae       ~[*via]
...
++  ineo                           :: handle terminal input
  ...
  ?-  -.mos.ego
    ...
    =/  lex  (~(get by element.omen.ego) (noto zon))
    ?~  lex  [~ ego]
    %^    muto
        zon
      u.lex
  ...
  ==
::
++  bibo                           :: get session source from a bowl
  |=  bol=bowl:gall
  ^-  fons
...
::
++  scio                           :: construct keybinding state
  ^-  omen
  =|  =omen
  %_  omen
    element
      %-  malt
      ^-  (list [nota element:lex])
      ...
      ;*  ?+  -.mos.ego  ~
            ...
            ;=  ;row(h "1", px "1", fg "#EBF2FA", bg "#064789"):"%editor"
                ;row(w "grow", h "1", pr "1", bg "#427AA1", fx "end")
                  ;+  ;/  ?~(count.mos.ego ~ ((d-co:co 1) u.count.mos.ego))
                ==
...
++  apto                           :: reevaluate a frame
  ...
  =:  arae.via.arx.urbs.ego
      ...
      arae.via
        ...
        |=  [=ara i=@]
        :_  +(i)
        (curo (container:eruo i) (~(got by containers) fons.ara) ara)
    ==
...
++  moto                           :: motion core
  ...
  ++  lavo                         :: recompute the editor viewport start position; finalize
    ...
    ?:  (gte rows-total target)
      %_  fav
        lin.flos  line-count
        row.flos  (sub rows-total target)
      ==
    ?:  =(0 line-count)
      %_  fav
        flos  [1 0]
    ...
    ==
  ::
  ++  luo                          :: determine if the cursor is out of bounds
    ^-  ?(%up %down %$)
    =/  thresh           tuto
  ...
  ++  tuto                         :: get the threshold size by viewport height
    ^-  @
    =/  percent  20
    (div (mul percent viewport-height) 100)
  ::
    ...
    =.  mos.ego
      ...
      %_  mos.ego
        count
          ...
          %-  scan
          :_  dim:ag
          =/  cun  ?~(count.mos.ego ~ ((d-co:co 1) u.count.mos.ego))
          ?+  -.zon  !!
            %chr  (snoc cun (tuft p.zon))
            %txt  (weld cun (tufa p.zon))
          ==
      ==
    =.  deus.urbs.ego  status:sys:velo
    :_  ego
...
++  fuco                           :: cover a given area with a pattern
  ...
  $(y +(y), iy +(iy), bas t.bas)
::
++  filo                           :: resolve an element's style
